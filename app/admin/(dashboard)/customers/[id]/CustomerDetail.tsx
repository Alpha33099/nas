...
export default function CustomerDetail({
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      {/* ── Location & Anti-Fraud Intelligence Action Card ──── */}
      <div className="bg-white rounded-2xl border border-slate-200/90 p-5 sm:p-6 shadow-2xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-start gap-3.5">
            <div className="w-10 h-10 rounded-xl bg-teal-50 flex items-center justify-center text-teal-600 border border-teal-200/60 shrink-0 mt-0.5">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                ) : customer.last_login_city ? (
                  <span className="text-2xs font-medium bg-slate-100 text-slate-600 border border-slate-200 px-2 py-0.5 rounded-md">
                    🌐 IP Geolocation
                  </span>
                ) : null}
                {customer.first_login_country && customer.last_login_country && customer.first_login_country !== customer.last_login_country && (
              ...
              <p className="text-xs text-slate-600 mt-1">
                {customer.last_login_city || customer.first_login_city ? (
                  <span>
                    📍 Last active from <strong>{customer.last_login_locality ? `${customer.last_login_locality}, ` : ""}{customer.last_login_city || customer.first_login_city}, {customer.last_login_country || customer.first_login_country}</strong>
                    {customer.last_login_ip && <span className="font-mono text-slate-400 ml-1.5">({customer.last_login_ip})</span>}
                  </span>
                ) : (
                  <span className="text-slate-400">No login recorded yet. Location will anchor upon first login.</span>
                )}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5 self-start sm:self-auto shrink-0">
            ...
            <Link
              href={`/admin/customers/${customer.id}/location`}
              className="inline-flex items-center gap-1.5 px-4 py-2 bg-teal-600 hover:bg-teal-700 active:bg-teal-800 text-white text-xs font-bold rounded-xl shadow-md shadow-teal-600/20 transition-all"
            >
              <span>View Location & Security Details</span>

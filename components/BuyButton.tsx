  ...
  async function handleBuyClick() {
    ...
    try {
      ...
      if (data.loggedIn && data.customer) {
        ...
        // Unauthenticated guests redirect to login as specified
    ...
    <>
      <div className="flex gap-2">
        <button
          ...
          className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-xl bg-teal-600 hover:bg-teal-700 active:bg-teal-800 px-4 py-2.5 text-xs sm:text-sm font-bold text-white transition shadow-xs shadow-teal-600/20 disabled:opacity-70"
        >
          <Zap size={14} className="fill-white" />
          <span>{checkingAuth ? "Checking..." : "BUY NOW"}</span>
        </button>
        <button
          type="button"
